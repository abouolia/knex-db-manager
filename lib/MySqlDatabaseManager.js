...
/**
 * Check the given database if was exist in the database. 
 */
MySqlDatabaseManager.prototype.existDb = function(databaseName){
  return this._masterQuery("SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = '??'", [databaseName])
    .then(result => {
      let exist = _.find(result, schema => schema.SCHEMA_NAME === databaseName) !== undefined;
      return exist;
    });
};
